...
Flood hazard modeling involves estimating the spatial extent and intensity of flood events for given probabilities or return periods. It forms the foundational input to any flood risk assessment, determining where and how deep flooding occurs under specific scenarios. Two main categories of models are commonly used: global-scale models and high-resolution local hydraulic models.
...
One such model is the \textbf{WRI Aqueduct River Flood Model}, developed by the World Resources Institute. It provides flood depth estimates at approximately 1 km resolution for return periods ranging from 2 to 1000 years and supports climate change scenarios such as RCP4.5 and RCP8.5 \cite{wri_aqueduct_floods, ward_aqueduct_2020}. The model is designed to support scenario-based stress testing, risk disclosures, and portfolio-level flood exposure estimation. However, its spatial granularity may be insufficient for site-level decision-making or building-level risk evaluation \cite{woetzel2020climate, ipcc2021}.


\subsection{Local Hydraulic Models}

In contrast, local hydraulic models are developed with detailed topographic, land use, and hydraulic calibration data for specific regions. Slovenia’s national flood hazard dataset—the \textbf{Integralna karta globin (IKG)}—is a prominent example. 

IKG is commissioned by the Ministry of Natural Resources and Spatial Planning, specifically its Water Directorate (\textit{Direkcija Republike Slovenije za vode}), and modeled by certified engineering firms such as DRI d.o.o., iS Projekt, and the Institute for Water of the Republic of Slovenia. These models are part of Slovenia’s implementation of the EU Floods Directive (2007/60/EC) and follow a standardized methodology set forth in national legislation \cite{noauthor_pravilnik_nodate}.

Flood simulations are based on 1D or 2D hydraulic modeling using high-resolution LiDAR-derived terrain data. The outputs classify inundation into three discrete depth categories:
\begin{itemize}
...
These flood zones are available for return periods of 10, 100, and 500 years and are published as vector geospatial layers on the Slovenian open data portal \cite{ikg_slovenia_data_2025}. Their main regulatory purpose is to inform land-use planning, assess flood risk for development projects, and support disaster risk reduction.

\textit{Use case description from official portal (translated) \cite{ikg_slovenia_data_2025}}:
\begin{quote}
