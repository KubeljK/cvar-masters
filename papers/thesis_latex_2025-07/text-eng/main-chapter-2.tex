...
Flood hazard modeling involves estimating the spatial extent and intensity of flood events for given probabilities or return periods. It forms the foundational input to any flood risk assessment, determining where and how deep flooding occurs under specific scenarios. Two main categories of models are commonly used: global models and high-resolution local hydraulic models.

\subsection{Global Flood Models}

Global flood models are designed for scalability and global coverage. They typically simulate flood hazard using simplified hydrological and hydraulic models driven by global datasets such as digital elevation models, river discharge records, and climate projections. A widely used example is the \textbf{WRI Aqueduct River Flood model}, which provides fluvial flood depth maps for return periods ranging from 2 to 1000 years, with a spatial resolution of ~1 km and support for climate change scenarios such as RCP4.5 and RCP8.5 \cite{wri_aqueduct_floods, ward_aqueduct_2020}.

These models are designed to support large-scale comparisons, stress testing, and financial disclosures. However, their coarse spatial resolution limits their application in local-level asset risk estimation or infrastructure planning \cite{woetzel2020climate, ipcc2021}.

\subsection{Local Hydraulic Models}

In contrast, local hydraulic models such as Slovenia’s \textbf{Integralna karta globin (IKG)} are developed with detailed topography, land use, and hydraulic calibration. These models often rely on 1D or 2D hydraulic simulations and historical flood data to generate detailed flood hazard maps for selected municipalities. In the case of IKG, flood depth is categorized into discrete classes (GM, GS, GB), corresponding to increasing flood severity, and available for return periods of 10, 100, and 500 years \cite{ikg_slovenia_2023, noauthor_karte_nodate}.

While local models lack global coverage and scenario flexibility, they provide high spatial precision (typically 5–10 m resolution) that is critical for building-level flood risk modeling and regulatory planning.

\subsection{Return Periods and Exceedance Probabilities}

Both global and local models represent flood hazard as a function of return period \( T \), which reflects the average time between flood events of a given magnitude. The annual exceedance probability (AEP) is defined as:
...
At the national level, ministries of the environment and civil protection agencies—such as the \textbf{Uprava RS za zaščito in reševanje} in Slovenia—are responsible for developing flood hazard maps, collecting damage reports, and designating flood risk zones under the EU Floods Directive. These data and designations serve as critical inputs for both public and private sector climate risk assessments \cite{ikg_slovenia_2023, urszr_damage_data}.
